...
Interviewee number eight stated that if there appears any issue with Päikky, they usually tries to find someone else to solve it. Other interviewees could not remember an issue with Päikky that they could not handle. Only exception for this was the case on the other kindergarten's devices, which would seemingly load Päikky everlastingly on the start. On situations like that the loader icon would not disappear after user started Kindergarten UI from the home menu, making the using of the application impossible. This was fixed on an update to the system later on.
